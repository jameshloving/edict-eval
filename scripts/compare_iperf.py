...
def get_data(dirs, header_len):
    ...
    for d in dirs:
        ...
        #    for line in f:
        #        if line.split()[0] == '[SUM]':
        #            entry['v4'].append(float(line.split()[-2]))
        #            half_total = float(line.split()[-2])

        #infile = d + 'iperf6.txt'
        #with open(infile) as f:
        #    for line in f:
        #        if line.split()[0] == '[SUM]':
        #            entry['v6'].append(float(line.split()[-2]))
        #            half_total += float(line.split()[-2])
        #            entry['total'].append(half_total)

        infile = d + 'iperf6.txt'
        with open(infile) as f:
            for line in f:
                if line != '\n' and line.split()[0] == '[SUM]':
                    entry['v6'].append(float(line.split()[line.split().index('Mbits/sec')-1]))
